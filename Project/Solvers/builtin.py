...
def build_vals(c,nx,ny,dx,dy,D,z,num_sources,sc,size2,k):
    ...
    Nxv = Nx.reshape((4, ny, nx))
    Nyv = Ny.reshape((4, ny, nx))

    dnxdy, dnxdx = np.gradient(Nxv, dy, dx, axis=(1, 2))
    dnydy, dnydx = np.gradient(Nyv, dy, dx, axis=(1, 2))

    Nxx = dnxdx.flatten()
    Nyy = dnydy.flatten()

    #r_temp = -k*c[:size2]*c[size2:2*size2]
    #r_zero = np.zeros_like(r_temp)

    #r = np.concatenate([r_temp,r_temp,r_zero,r_zero])
    #r[mask] = 0
    #plot_initial(r,nx,ny,0)
    r = build_reaction(c,k,nx,ny,sc,num_sources)

    return r,Nxx,Nyy,Nx,Ny,dcdx,dcdy #Nxx,Nyy,Nx,Ny,dcdx,dcdy
...
def solver(c,n,Nx,Ny,nx,ny,dx,dy,sc,num_sources,k,D,z,sources,source_size,num_particles,rad,psi_D,Dp,dt,realtime,max_time,start_time):
    ...
    while time < max_time:
        ...
        err_old = 100
        c_old = np.copy(c)
        r,Nxx,Nyy,Nx,Ny,dcdx,dcdy = build_vals(c,nx,ny,dx,dy,D,z,num_sources,sc,size2,k)
        inner_iter = 0
        while err > 1e-12 and err_change < 0: #or np.min(c) < 0:
            #plot_initial(c,nx,ny,0)
            #plot_initial(Nx,nx,ny,1)
            #plot_initial(Ny,nx,ny,2)
            #plot_initial(c,nx,ny,0)
            #plot_initial(Nxx,nx,ny,1)
            #plot_initial(Nyy,nx,ny,2)
            c_previous = c
            def c_sys(x):
                nonlocal r,Nxx,Nyy,Nx,Ny,dcdx,dcdy
                return (Nxx + Nyy)*dt - r*dt - c_old + x          
            c = newton_krylov(c_sys,c,iter=1,method='lgmres')
            r,Nxx,Nyy,Nx,Ny,dcdx,dcdy = build_vals(c,nx,ny,dx,dy,D,z,num_sources,sc,size2,k)
            err = np.linalg.norm((Nxx + Nyy)*dt - r*dt - c_old + c)/size
        ...
        if err_change < 0 and inner_iter != 1:
            c = c_previous
